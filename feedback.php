<?php
	...
	if(isset($_POST['name']) && isset($_POST['email']) && isset($_POST['course']) &&isset($_POST['company']) && isset($_POST['contact']) && isset($_POST['submission1'])&& isset($_POST['question'])){
		...
		/*File validations*/
			...
			if(!$name_flag && !$contact_flag && !$file_flag && !$feedback_flag &&!empty($course)&&!empty($company)&&!empty($email) ){
			...
			if($handle =  fopen('feedback.txt' , 'a')){
				...
				if($query_run = mysql_query($query_check)){
					...
					$query = "INSERT INTO feedback(`name` , `course` , `email` , `contact` , `company`) VALUES('$name' , '$course' , '$email' ,'$contact' ,'$company')";
...
<html>
	...
	<div class = "ui error form segment" >
		<div class="ui error message" id = "errorbox">
	...
	if(isset($_POST['captcha'])){
		if(!($_POST['captcha'] == $_SESSION['secure'])){
			...
			echo '<div class="ui error message">
						<div class="header">Invalid CAPTCHA CODE</div>
						<p>Please Try Again</p>
					</div>';
		}
		else if($name_flag || $file_flag || $contact_flag =0){
			$_SESSION['secure'] = rand(1000 , 9999);
		}
